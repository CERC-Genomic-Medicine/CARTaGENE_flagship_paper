...
process impute {
    ...
    if [ "${ref_prefix}" = "chrX_par1" ]; then
       ...
       ${params.minimac4} ${ref_msav} temp.bcf -t 8 -f GT,DS,HDS -o imputed_noinfo.dose.bcf -e ${ref_prefix}.imputed.empirical_dose.vcf.gz
       ...
       ${params.minimac4} ${ref_msav} ${study_vcf} -t 8 -f GT,DS,HDS -o imputed_noinfo.dose.bcf -e ${ref_prefix}.imputed.empirical_dose.vcf.gz
    fi
    
    # Index
    bcftools index imputed_noinfo.dose.bcf
    bcftools index ${ref_prefix}.imputed.empirical_dose.vcf.gz

    # Add back INFO fields and relevant meta-lines for the SVs
    bcftools annotate -a ${ref_msav_info} -h ${ref_msav_info_header} -c CHROM,POS,~ID,REF,ALT,INFO/END,INFO/SVTYPE,INFO/SVLEN imputed_noinfo.dose.bcf -Oz -o ${ref_prefix}.imputed.dose.vcf.gz
    bcftools index -t ${ref_prefix}.imputed.dose.vcf.gz
    """
}
